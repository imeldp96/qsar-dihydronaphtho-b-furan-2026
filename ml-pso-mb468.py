...
# =========================================
data = pd.read_csv('./mb468.csv', index_col=[0])
data = data.fillna(0)
...
X_train = X_train_fi[['BCUTdv-1l', 'VSA_EState3', 'AETA_eta_L']] #pi-gbr
X_test = X_test_fi[['BCUTdv-1l', 'VSA_EState3', 'AETA_eta_L']]
...
base_pos=np.array([1.98177673e+02, 8.28575762e+00, 1.53100716e+00, 1.10451579e-02])
n_particles = 250
dimensions = 4
...
rf_optimizer = ps.single.GlobalBestPSO(n_particles=n_particles, dimensions=dimensions, 
...
base_pos=np.array([2.90515098, 0.16858201])
n_particles = 250
dimensions = 2
noise_scale = 0.00  # adjust as needed
init_pos = np.tile(base_pos, (n_particles, 1)) + np.random.uniform(-noise_scale, noise_scale, size=(n_particles, dimensions))
...
svr_optimizer = ps.single.GlobalBestPSO(n_particles = n_particles, dimensions=dimensions,
...
base_pos=np.array([14.66581423, 3.8233361, 0.29001192, 1.35566015, 0.98951206])
n_particles = 250
dimensions = 5
noise_scale = 0.00  # adjust as needed
init_pos = np.tile(base_pos, (n_particles, 1)) + np.random.uniform(-noise_scale, noise_scale, size=(n_particles, dimensions))
...
gbr_optimizer=ps.single.GlobalBestPSO(n_particles = n_particles, dimensions=dimensions,
                                      options={'c1':0.5,'c2':0.3,'w':0.9},
...
base_pos=np.array([0.79643978, 0.06937038, 13.9853997, 0.94777127, 0.96689066, 0.75748807,
  1.57247124, 0.03781847])
n_particles = 250
dimensions = 8
noise_scale = 0.00  # adjust as needed
init_pos = np.tile(base_pos, (n_particles, 1)) + np.random.uniform(-noise_scale, noise_scale, size=(n_particles, dimensions))
...
xgb_optimizer=ps.single.GlobalBestPSO(n_particles = n_particles, dimensions=dimensions,
                                      options={'c1':0.5,'c2':0.3,'w':0.9},
...
base_pos=np.array([3.08633737e+02, 4.48701214e+00, 6.99803930e+04, 8.15061612e-02,
 4.64091107e+04])
n_particles = 250
dimensions = 5
noise_scale = 0.00  # adjust as needed
...
mlp_optimizer=ps.single.GlobalBestPSO(n_particles = n_particles,dimensions=dimensions,
                                      options={'c1':0.5,'c2':0.3,'w':0.9},
...
# Selected features
feature_names = ['BCUTdv-1l', 'VSA_EState3', 'AETA_eta_L']
...
all_df.to_csv('all_data_mdamb468-121025.csv', index=False)
...
plt.savefig('plot_expvspred_mdamb468-121025.tif', dpi=300, format='tif')
plt.show()
...
results_df.to_csv("validation_results_loocv_lmocv-mb468-121025.csv", index=False)
print(results_df)